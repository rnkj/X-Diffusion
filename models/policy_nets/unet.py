...
class HalfUnet1D(nn.Module):
    ...
    def forward(
        ...
        timestep: (B,) or int, diffusion step
        cond: (B, global_cond_dim) or None
        output: (B,C,T) latent features from encoder+bottleneck
        """
        # (B,T,C) -> (B,C,T)
        sample = sample.moveaxis(-1, -2)
        ...
        # NOTE: we don’t do decoder or final_conv
        # Return (B,C,T) latent features
        return x
